# AUTOGENERATED! DO NOT EDIT! File to edit: develop/utils/env_setup.ipynb (unless otherwise specified).

__all__ = ['setup_musescore', 'is_ipython', 'is_ipython', 'is_colab', 'setup_fluidsynth', 'play_wav']
...
def setup_musescore(musescore_path=None):
    ...
    if not settings.getSettingsPath().exists(): settings.create()

    if settings['musicxmlPath'] and settings['musescoreDirectPNGPath']: return
    if musescore_path is None:
        if system == 'Darwin':
            ...
            musescore_path = '/usr/bin/mscore3' if Path('/usr/bin/mscore3').exists() else '/usr/bin/musescore'
    ...
    if musescore_path is None or not Path(musescore_path).exists():
        ...
        settings['musicxmlPath'] = musescore_path
        settings['musescoreDirectPNGPath'] = musescore_path
