...
__all__ = ['MTTTransformer', 'reset_children', 'TransformerEmbedding', 'MTTLinearDecoder', 'MTTEncoder',
           'MTTEncoderBlock', 'MemMultiHeadRelativeAttentionKV']

# Cell
from typing import Collection, Tuple
from fastai.basics import *
from ..utils.fastai_transformer import *
from ..utils.fastai_transformer import _line_shift
from ..utils.attention_mask import *

# Cell
class MTTTransformer(nn.Module):
    ...
    @classmethod
    def from_config(cls, vocab_size:int, config:dict=None, drop_mult:float=1., pad_idx=None):
        "Create a language model its `config`."
        ...
        encoder = MTTEncoder(embed, n_hid, n_layers=config['enc_layers'], mem_len=0, **config) # encoder doesn't need memory
        decoder = MTTEncoder(embed, n_hid, is_decoder=True, n_layers=config['dec_layers'], mem_len=mem_len, **config)
        head = MTTLinearDecoder(n_hid, vocab_size, tie_encoder=embed.embed, **config)
        model = cls(encoder, decoder, head, mem_len=mem_len)
...
class MTTLinearDecoder(nn.Module):
...
class MTTEncoder(nn.Module):
    def __init__(self, embed:nn.Module, n_hid:int, n_layers:int, n_heads:int, d_model:int, d_head:int, d_inner:int,
        ...
        self.layers = nn.ModuleList([MTTEncoderBlock(n_heads, d_model, d_head, d_inner, resid_p=resid_p, attn_p=attn_p,
...
class MTTEncoderBlock(nn.Module):
