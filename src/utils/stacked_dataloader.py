...
class StackedDL():
    def __init__(self, dls, num_it=100):
        self.dls = dls
        self.dataset = StackedDS([dl.dataset for dl in dls if hasattr(dl, 'dataset')])
        self.num_it = num_it
        self.dl_idx = -1
...
class StackedData():
    def __init__(self, dbs, num_it=100):
        self.dbs = dbs
        self.train_dl = StackedDL([db.train_dl for db in self.dbs], num_it)
        self.valid_dl = StackedDL([db.valid_dl for db in self.dbs], num_it)
        self.vocab = vocab
        self.train_ds = None
