...
class OpenAIGPTLMHead(nn.Module):
    ...
    def set_embeddings_weights(self, model_embeddings_weights):
        vocab_size = self.config.vocab_size + self.config.n_special
        decoder_weights = torch.nn.Parameter(model_embeddings_weights[:vocab_size])
        embed_shape = decoder_weights.shape
        self.decoder = nn.Linear(embed_shape[1], embed_shape[0], bias=False)
        self.decoder.weight = decoder_weights # Tied weights
